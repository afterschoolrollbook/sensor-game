  ...
  document.getElementById('ptarea').innerHTML=`
    <div class="pa">
      ...
      <div style="display:flex;gap:8px;">
        <button class="bxls" style="flex:1;" onclick="impXlsNew()">📂 CSV/엑셀에서 불러오기</button>
        <button class="bxls" style="border-color:var(--red);color:var(--red);" onclick="clearAllPts()">🗑️ 전체 삭제</button>
      </div>
    </div>`;
  renderPL();
}
...
function clearAllPts(){
  if(!S.pts.length){toast('삭제할 참가자가 없어요','info');return;}
  if(!confirm(S.pts.length+'명 전체를 삭제할까요?'))return;
  S.pts=[];
  renderPL();updatePv();
  try{App.setState({participants:S.pts});}catch(e){}
